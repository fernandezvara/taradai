...
class PortraitUploader < CarrierWave::Uploader::Base
  ...
  def default_url
   "/images/portal/avatars/" + [version_name, "default.png"].compact.join('_')
  end

  def url
    if model.portrait_filename.nil? == true or model.portrait_filename == ""
      return "#{default_url}"
    else
      prepend1 = model.id.to_s.split('')[8].to_s
      prepend12 = "#{model.id.to_s.split('')[0].to_s}#{model.id.to_s.split('')[1].to_s}#{model.id.to_s.split('')[2].to_s}"
      ...
      case prepend1
      when "1", "5", "9", "c"
        "http://assets1.taradai.com/portrait/#{prepend12}/#{prepend34}/#{prepend56}/" + [version_name, "#{filename}"].compact.join('_')
      when "2", "6", "0", "d"
        "http://assets2.taradai.com/portrait/#{prepend12}/#{prepend34}/#{prepend56}/" + [version_name, "#{filename}"].compact.join('_')
      when "3", "7", "a", "e"
        "http://assets3.taradai.com/portrait/#{prepend12}/#{prepend34}/#{prepend56}/" + [version_name, "#{filename}"].compact.join('_')
      when "4", "8", "b", "f"
        "http://assets4.taradai.com/portrait/#{prepend12}/#{prepend34}/#{prepend56}/" + [version_name, "#{filename}"].compact.join('_')
      end
    end
  end
